#!/bin/bash
# FireHydrant ETL cron wrapper.
#
# Cron entry (daily at 8:20am):
#   20 8 * * * /path/to/developer-cycle-time-dashboard/scripts/run_firehydrant_etl.sh

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
LOG_DIR="$PROJECT_DIR/data/logs"
LOG_FILE="$LOG_DIR/firehydrant_etl.log"
ERROR_FILE="$LOG_DIR/firehydrant_etl_errors.log"
...
"$PYTHON" "$PROJECT_DIR/firehydrant_etl.py" >> "$LOG_FILE" 2>&1
EXIT_CODE=$?
